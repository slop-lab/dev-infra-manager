...
helper="$repo_root/.agents/skills/pull-request/scripts/forge-pr.bash"
...
github="$(cd "$work_dir/github" && PATH="$work_dir/bin:$PATH" bash "$helper" detect)"
...
gitea="$(cd "$work_dir/gitea" && PATH="$work_dir/bin:$PATH" bash "$helper" detect)"
