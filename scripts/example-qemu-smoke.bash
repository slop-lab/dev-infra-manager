...
printf 'yes\n' | ssh "${ssh_args[@]}" dim@127.0.0.1 \
  "cd dim && bash scripts/install-host-ubuntu.bash '$backend'" >/dev/null
ssh "${ssh_args[@]}" dim@127.0.0.1 '
  set -e
...
ssh "${ssh_args[@]}" dim@127.0.0.1 \
  "cd dim && just install && $guest_environment just verify-example current-installed auto '$selection'"
