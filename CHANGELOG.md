...
- Initialized the canonical rootless-DinD sidecar's volume-mounted data path,
  ...
  plus validation and recovery of partial prior repairs, avoiding nested-runc
  failures when a volume does not inherit image ownership.
...
- Added Project-owned streaming `backup` and `restore` tasks to the canonical
  ...
  for a consistent snapshot and use a networkless temporary container with
...
- Recreated canonical self-Project containers during setup and restored the
  ...
  Setup now rebuilds the sidecar image explicitly so existing broken
  workspaces can adopt the repair in place.
...
- Made the installer a strict facade: it owns only `installer`,
  ...
  [Installer Facade](specification/specs/14-installer-facade.md).
...
- Renamed the unprivileged account used by workspace-root images from `agent`
  ...
  See [Trust Boundaries](specification/specs/02-boundaries-and-trust.md#agent-container-boundary).
...
- Split the root `README.md` into user-facing content (installing and using
  ...
  ([docs/README.md](specification/docs/README.md#glossary)) covering Project/Workspace
...
- Every documented walkthrough (root README, `specification/docs/repo-workspaces.md`,
  `specification/docs/project-workspaces.md`, `specification/docs/usage.md`, `core/packages/cli/README.md`,
