...
export const SYSBOX_CI_RUNNER_IMAGE =
  "dev-infra-manager-ci-runner:act-runner-node-24.19.0-just-1.40.0-jq-socat";

export const SYSBOX_CI_RUNNER_DOCKERFILE = `FROM ${SYSBOX_CI_RUNNER_BASE_IMAGE}
RUN apk add --no-cache jq just=1.40.0-r0 libgcc=14.2.0-r6 libstdc++=14.2.0-r6 socat \
...
RUN node --version && just --version && git --version && docker --version && act_runner --version
`;
