...
```text
$XDG_DATA_HOME/dim/cli/0.5.0/node_modules/.bin/dim
```

(falling back to `~/.local/share/dim/cli/0.5.0/...` when `XDG_DATA_HOME` is
...
```text
~/.local/bin/dim -> $XDG_DATA_HOME/dim/cli/0.5.0/node_modules/.bin/dim
```
...
- **Installed**: `dim --help` is forwarded to the real CLI's own `--help`.
  `dim --version` prints:
  ```text
  DIM CLI 0.5.0 (via DIM installer 0.5.0)
  ```
