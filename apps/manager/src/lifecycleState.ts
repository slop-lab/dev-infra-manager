...
export class LifecycleState {
  ...
  async readWorkspace(name: string): Promise<WorkspaceRecord> {
    return readJson(this.workspacePath(name), `workspace '${name}' not found`);
  }
  ...
  async acquireWorkspaceLock(name: string): Promise<() => Promise<void>> {
    const lockPath = path.join(this.root, "locks", `workspace-${validateLifecycleName(name, "workspace")}.lock`);
    ...
    throw new UserError(`timed out waiting for workspace '${name}' reconciliation lock`);
  }
